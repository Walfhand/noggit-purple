...
bool World::paintGroundEffect(glm::vec3 const& pos, float radius, std::string const& texture_filename, unsigned int effect_id, bool overwrite)
{
  ZoneScoped;
  return for_all_chunks_in_range
    (pos, radius
      , [&](MapChunk* chunk)
      {
        if (!chunk->canSetGroundEffectForTexture(texture_filename, effect_id, overwrite))
        {
          return false;
        }

        NOGGIT_CUR_ACTION->registerChunkLayerInfoChange(chunk);
        return chunk->setGroundEffectForTexture(texture_filename, effect_id, overwrite);
      }
    );
}
