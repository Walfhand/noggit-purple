...
bool TextureSet::canSetEffectForTexture(std::string const& texture_filename, unsigned int value, bool overwrite) const
{
  for (std::size_t i = 0; i < nTextures; ++i)
  {
    if (textures[i]->file_key().filepath() != texture_filename)
    {
      continue;
    }

    unsigned int const current_effect = _layers_info[i].effectID;
    if (!overwrite && current_effect && current_effect != 0xFFFFFFFF)
    {
      continue;
    }

    if (current_effect != value)
    {
      return true;
    }
  }
...
bool TextureSet::setEffectForTexture(std::string const& texture_filename, unsigned int value, bool overwrite)
{
  bool changed = false;
...
bool TextureSet::setEffect(size_t id, unsigned int value)
{
  if (id >= nTextures || _layers_info[id].effectID == value)
  {
    return false;
  }

  _layers_info[id].effectID = value;
  _chunk->registerChunkUpdate(ChunkUpdateFlags::FLAGS | ChunkUpdateFlags::GROUND_EFFECT);
  return true;
}
