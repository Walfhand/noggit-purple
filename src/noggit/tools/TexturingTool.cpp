    ...
    void TexturingTool::onTick(float deltaTime, TickParameters const& params)
    {
        ...
        if (_texturingTool->getTexturingMode() == Noggit::Ui::texturing_mode::ground_effect)
        {
            if (params.mod_shift_down)
            {
                ...
                {
                    auto effect_id = _texturingTool->getGroundEffectsTool()->selectedGroundEffectId();
                    std::string const active_texture = _texturingTool->_current_texture->filename();
                    if (effect_id.has_value() && !active_texture.empty() && active_texture != "tileset\\generic\\black.blp")
                    {
                        NOGGIT_ACTION_MGR->beginAction(mv, Noggit::ActionFlags::eCHUNKS_LAYERINFO,
                            Noggit::ActionModalityControllers::eSHIFT
                            | Noggit::ActionModalityControllers::eLMB);
                        if (mv->getWorld()->paintGroundEffect(mv->cursorPosition(),
                            _texturingTool->getGroundEffectsTool()->radius(),
                            active_texture,
                            effect_id.value(),
                            _texturingTool->getGroundEffectsTool()->override_existing_effects()))
                        {
                            _texturingTool->getGroundEffectsTool()->refreshRenderOverlay();
                        }
                    }
                }
