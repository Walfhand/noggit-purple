...
namespace Noggit
{
    namespace Ui
    {
        namespace
        {
            bool is_valid_ground_effect_texture(std::string const& texture)
            {
                return !texture.empty() && texture != "tileset\\generic\\black.blp";
            }

            unsigned int zone_id_for_area(unsigned int area_id)
            {
                unsigned int const parent = AreaDB::get_area_parent(area_id);
                return parent ? parent : area_id;
            }

            float positive_fraction(float value)
            {
                return value - std::floor(value);
            }

            QString ground_effect_label(ground_effect_set const& effect)
            {
                QString label = QString("ID %1").arg(effect.ID);
                for (auto const& doodad : effect.Doodads)
                {
                    if (!doodad.filename.empty())
                    {
                        label += QString(" - %1").arg(QFileInfo(QString::fromStdString(doodad.filename)).fileName());
                        break;
                    }
                }
            ...
            : QWidget(parent, Qt::Window), _texturing_tool(texturing_tool), _map_view(map_view)
        {
            ...
            auto effect_id_layout = new QHBoxLayout();
            effect_id_layout->addWidget(new QLabel("GroundEffectTexture ID:", this));
            _effect_id_spinbox = new QSpinBox(this);
            _effect_id_spinbox->setRange(1, 2147483647);
            effect_id_layout->addWidget(_effect_id_spinbox);
            auto button_create_new = new QPushButton("Add DBC ID", this);
            effect_id_layout->addWidget(button_create_new);
            selection_layout->addLayout(effect_id_layout);

            auto button_load_dbc = new QPushButton("Load DBC Effects", this);
            button_load_dbc->setToolTip("Load all valid GroundEffectTexture rows from the client DBC.");
            selection_layout->addWidget(button_load_dbc);
            ...
            _selection_status_label = new QLabel("Load or scan a ground effect set.", this);
            _selection_status_label->setWordWrap(true);
            selection_layout->addWidget(_selection_status_label);
            ...
            // Effect settings.
            {
                ...
                for (auto it = gTerrainTypeDB.begin(); it != gTerrainTypeDB.end(); ++it)
                {
                    ...
                    _cbbox_terrain_type->setItemData(_cbbox_terrain_type->count() - 1, QVariant(terrain_type_record.getUInt(TerrainTypeDB::TerrainId)));
                }
                ...
                button_save_settings->setToolTip("DBC writing is not implemented. Edit your custom DBC patch, then load the GroundEffectTexture ID here.");
            }
            ...
            QRadioButton* generate_effect_zone = nullptr;
            QRadioButton* generate_effect_area = nullptr;
            QRadioButton* generate_effect_adt = nullptr;
            QRadioButton* generate_effect_loaded_tiles = nullptr;
            ...
            // Generate modes.
            {
                ...
                generate_effect_zone = new QRadioButton("Current Zone", this);
                generate_effect_zone->setToolTip("Apply on loaded chunks whose area belongs to the current zone.");
                generate_type_group->addButton(generate_effect_zone);
                buttons_layout->addWidget(generate_effect_zone, 0, 0);

                generate_effect_area = new QRadioButton("Current Area (Subzone)", this);
                generate_effect_area->setToolTip("Apply on loaded chunks with the same AreaID as the current camera position.");
                generate_type_group->addButton(generate_effect_area);
                buttons_layout->addWidget(generate_effect_area, 0, 1);

                generate_effect_adt = new QRadioButton("Current ADT (Tile)", this);
                generate_type_group->addButton(generate_effect_adt);
                buttons_layout->addWidget(generate_effect_adt, 1, 0);

                generate_effect_loaded_tiles = new QRadioButton("Loaded Tiles", this);
                generate_effect_loaded_tiles->setToolTip("Apply on all currently loaded ADTs. This avoids loading the whole map into memory.");
                generate_type_group->addButton(generate_effect_loaded_tiles);
                buttons_layout->addWidget(generate_effect_loaded_tiles, 1, 1);
            ...
            connect(button_create_new, &QPushButton::clicked,
                [this]()
                {
                    addGroundEffectById(static_cast<unsigned int>(_effect_id_spinbox->value()));
                });

            connect(button_load_dbc, &QPushButton::clicked,
                [this]()
                {
                    loadGroundEffectsFromDbc();
                });

            connect(button_generate, &QPushButton::clicked,
                [=]()
                {
                    ground_effect_apply_scope scope = ground_effect_apply_scope::current_zone;
                    if (generate_effect_area->isChecked())
                    {
                        scope = ground_effect_apply_scope::current_area;
                    }
                    else if (generate_effect_adt->isChecked())
                    {
                        scope = ground_effect_apply_scope::current_adt;
                    }
                    else if (generate_effect_loaded_tiles->isChecked())
                    {
                        scope = ground_effect_apply_scope::loaded_tiles;
                    }

                    applyGroundEffectToTexture(scope);
                });
        ...
        void GroundEffectsTool::updateSetsList()
        {
            ...
            for (auto& effect_set : _loaded_effects)
            {
                ...
                QListWidgetItem* list_item = new QListWidgetItem(ground_effect_label(effect_set));
                _effect_sets_list->addItem(list_item);
                list_item->setData(Qt::UserRole, QVariant(effect_set.ID));
                list_item->setToolTip(QString("GroundEffectTexture ID %1").arg(effect_set.ID));
            ...
            auto first_item = _effect_sets_list->item(0);
        ...
        void GroundEffectsTool::genEffectColors()
        {
            ...
            for (auto& effect : _loaded_effects)
            {
                ...
                float r = positive_fraction(sin(glm::dot(glm::vec2(color_count), glm::vec2(12.9898f, 78.233f))) * 43758.5453f);
                float g = positive_fraction(sin(glm::dot(glm::vec2(color_count), glm::vec2(11.5591f, 70.233f))) * 43569.5451f);
                float b = positive_fraction(sin(glm::dot(glm::vec2(color_count), glm::vec2(13.1234f, 76.234f))) * 43765.5452f);
                color_count++;
        ...
        bool GroundEffectsTool::override_existing_effects() const
        {
          return _apply_override_cb->isChecked();
        }

        std::optional<unsigned int> GroundEffectsTool::selectedGroundEffectId()
        {
            auto effect = getSelectedGroundEffect();
            if (!effect.has_value())
            {
        ...
        void GroundEffectsTool::refreshRenderOverlay()
        {
            genEffectColors();
            updateTerrainUniformParams();
        }
        ...
        bool GroundEffectsTool::addGroundEffectById(unsigned int effect_id)
        {
            if (!gGroundEffectTextureDB.CheckIfIdExists(effect_id))
            {
                _selection_status_label->setText(QString("GroundEffectTexture ID %1 was not found in the loaded DBC.").arg(effect_id));
                return false;
            }
            ...
            if (effect.empty())
            {
                _selection_status_label->setText(QString("GroundEffectTexture ID %1 could not be loaded.").arg(effect_id));
                return false;
            }
            ...
            for (int i = 0; i < _loaded_effects.size(); ++i)
            {
                if (_loaded_effects[i].ID == effect.ID)
                {
                    target_index = i;
                    break;
                }
            ...
            if (target_index == -1)
            {
                ...
                target_index = static_cast<int>(_loaded_effects.size()) - 1;
                _selection_status_label->setText(QString("Added GroundEffectTexture ID %1.").arg(effect.ID));
            }
            else
            {
                _selection_status_label->setText(QString("GroundEffectTexture ID %1 is already loaded.").arg(effect.ID));
            }

            updateSetsList();
            if (target_index >= 0 && target_index < _effect_sets_list->count())
            {
                _effect_sets_list->setCurrentRow(target_index);
                setActiveGroundEffect(_loaded_effects[target_index]);
            }

            return true;
        }

        void GroundEffectsTool::loadGroundEffectsFromDbc()
        {
            int added = 0;
            int skipped = 0;
            ...
            for (auto it = gGroundEffectTextureDB.begin(); it != gGroundEffectTextureDB.end(); ++it)
            {
                ...
                for (auto const& loaded_effect : _loaded_effects)
                {
                    if (loaded_effect.ID == effect_id)
                    {
                        already_loaded = true;
                        break;
                    }
            ...
            updateSetsList();
            _selection_status_label->setText(QString("Loaded %1 GroundEffectTexture entries from DBC.").arg(added));
            if (!added && skipped)
            {
                _selection_status_label->setText("No new GroundEffectTexture entries were loaded from DBC.");
            }
        }

        void GroundEffectsTool::applyGroundEffectToTexture(ground_effect_apply_scope scope)
        {
            auto effect_id = selectedGroundEffectId();
            if (!effect_id.has_value())
            {
                return;
            }

            std::string const active_texture = _texturing_tool->_current_texture->filename();
            if (!is_valid_ground_effect_texture(active_texture))
            {
                return;
            }

            World* world = _map_view->getWorld();
            glm::vec3 const current_pos = _map_view->getCamera()->position;
            unsigned int const current_area = world->getAreaID(current_pos);
            unsigned int const current_zone = zone_id_for_area(current_area);
            std::vector<MapChunk*> chunks_to_change;

            auto chunk_in_scope = [&](MapChunk* chunk)
            {
                if (scope == ground_effect_apply_scope::current_adt || scope == ground_effect_apply_scope::loaded_tiles)
                {
                    return true;
                }

                unsigned int const chunk_area = chunk->getAreaID();
                if (scope == ground_effect_apply_scope::current_area)
                {
                    return chunk_area == current_area;
                }

                return zone_id_for_area(chunk_area) == current_zone;
            };

            auto collect_from_tile = [&](MapTile* tile)
            {
                ...
                for (int x = 0; x < 16; ++x)
                {
                    for (int y = 0; y < 16; ++y)
                    {
                        MapChunk* chunk = tile->getChunk(x, y);
                        if (!chunk_in_scope(chunk))
                        {
                            continue;
                        }

                        if (chunk->canSetGroundEffectForTexture(active_texture, effect_id.value(), override_existing_effects()))
                        {
                            chunks_to_change.push_back(chunk);
                        }
                    }
                }
            };

            if (scope == ground_effect_apply_scope::current_adt)
            {
                collect_from_tile(world->mapIndex.getTile(TileIndex(current_pos)));
            }
            else
            {
                for (MapTile* tile : world->mapIndex.loaded_tiles())
                {
                    collect_from_tile(tile);
                }
            }

            if (chunks_to_change.empty())
            {
                return;
            }

            NOGGIT_ACTION_MGR->beginAction(_map_view, Noggit::ActionFlags::eCHUNKS_LAYERINFO);
            for (MapChunk* chunk : chunks_to_change)
            {
                NOGGIT_CUR_ACTION->registerChunkLayerInfoChange(chunk);
                chunk->setGroundEffectForTexture(active_texture, effect_id.value(), override_existing_effects());
                world->mapIndex.setChanged(chunk->mt);
            }
            NOGGIT_ACTION_MGR->endAction();

            refreshRenderOverlay();
        }
        ...
        void GroundEffectsTool::setActiveGroundEffect(ground_effect_set const& effect)
        {
            ...
            _effect_id_spinbox->setValue(static_cast<int>(effect.ID));
            ...
            _cbbox_terrain_type->setCurrentIndex(terrain_index == -1 ? 0 : terrain_index);
